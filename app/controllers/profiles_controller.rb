class ProfilesController < ApplicationController
  ...
  def create
  	@profile = Profile.create(params.require(:profile).permit(:username, :location, :bio, :user_id, :avatar))
  	if @profile.save
  ...
  def update
    @current_user = User.find(session[:remember_token])
    @profile = Profile.where(:user_id => @current_user.id)
    if @profile.update(params.require(:profile).permit(:username, :location, :bio, :user_id, :avatar))
